...
@section('content')
  <x-container-admin-page>
    <x-slot:content>
      <x-title-admin-page title="Galeri Management" />

      <div class="w-full h-full flex justify-end">
        <a href="/galeries/create" class="hidden md:block lg:block w-fit h-fit px-5 py-2 bg-red-500 text-xs rounded-lg text-white">Tambah Data</a>
        <a href="/galeries/create" class="block md:hidden lg:hidden w-fit h-fit px-5 py-3 bg-red-500 text-xs rounded-lg text-white">
        ...
        @foreach ($galeries as $galery)
          <div class="grid gap-2 bg-white p-3 rounded-lg border shadow">
            <div class="w-full h-fit grid gap-3 mb-5">
              <img src="/image/{{ $galery->image }}" alt="" class="object-cover w-full h-52 rounded-lg">
            </div>
            <div class="w-full h-fit flex gap-3 items-center justify-end border-t border-[#ddd] pt-2">
              <div class="w-full h-fit">
                <p class="text-[9px]">{{ $galery->created_at->diffForHumans() }}</p>
              </div>
              <a href="{{ route('galeries.edit', $galery->id) }}"
