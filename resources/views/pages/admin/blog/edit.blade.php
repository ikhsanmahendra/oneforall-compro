...
@section('content')
  <x-container-admin-page>
    <x-slot:content>
      <x-title-admin-page title="Edit Data Blog" />
      ...
      <form action="{{ route('blogs.update', $blog->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="w-full h-full flex flex-col gap-5 mt-5">
          ...
          <div class="w-full h-fit flex flex-col gap-2">
            ...
            <img src="/image/{{ $blog->image }}" alt="" id="image-old" class="w-fit h-32 px-5 py-2 rounded-lg bg-white">
          ...
          <div class="w-full h-fit flex flex-col gap-2">
            ...
            <textarea name="body" id="" cols="30" rows="10"
              class="w-full h-fit px-5 py-2 rounded-lg bg-white focus:outline-none border border-gray-300">{{ $blog->body }}</textarea>
          </div>
          @error('body')
            <p class="text-red-500">{{ $message }}</p>
          @enderror
