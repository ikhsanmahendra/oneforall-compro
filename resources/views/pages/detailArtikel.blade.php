...
<div class="bg-white min-h-screen">
  <div class="w-full md:max-w-[90%] lg:max-w-[80%] h-full flex flex-col px-8 gap-5 my-10 lg:my-20 justify-center mx-auto lg:text-left text-center">
    ...
    <div class="w-full h-fit grid gap-7 my-10">
      <h1 class="font-bold text-4xl text-center capitalize">{{ $blog->title }}</h1>
      <span class="text-left text-xs text-[#63666A] font-medium">{{ $blog->created_at->format('d F Y') }}</span>
  
      <img src="{{ asset('image/' . $blog->image) }}" alt="" class="w-full h-96 object-cover rounded-xl">
      <h2 class="font-bold text-lg capitalize">{{ $blog->subtitle }}</h2>
